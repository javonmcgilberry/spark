...
export default async function HomePage() {
  ...
  const ctx = {env, managerSlackId: session.managerSlackId};
  let drafts: Awaited<ReturnType<typeof listDrafts>>['drafts'] = [];
  let publishedPackages: Awaited<
    ReturnType<typeof listDrafts>
  >['publishedPackages'] = [];
  ...
  try {
    ...
    drafts = listResult.drafts;
    publishedPackages = listResult.publishedPackages;
    me = profile;
  } catch (error) {
    fetchError =
      error instanceof SparkApiError
        ? `${error.status}: ${error.message}`
        : error instanceof Error
          ? error.message
          : 'Unknown error reaching Spark bot';
  }
  ...
  return (
    <div style={{maxWidth: 1080, margin: '0 auto', display: 'grid', gap: 24}}>
      <section style={panelStyle}>
        <p style={eyebrowStyle}>
          ...
          <strong style={{color: '#e2e8f0'}}>
            {me?.displayName ?? session.managerSlackId}
          </strong>
          {session.source === 'env' ? (
            <span
              style={{
      ...
      {fetchError ? (
        ...
        >
          <p style={sectionLabelStyle}>Bot unreachable</p>
