 ...
 * turn so tests can swap in a stub with a queued response list.
 */
export async function* runGenerator(
  ...
  const {
    ...
    model = ctx.env.ANTHROPIC_MODEL ?? DEFAULT_MODEL,
  } = options;
  ...
  for (; iteration < maxIterations; iteration++) {
    const response = await callLlmWithRetry(ctx, {
