...
export interface UserGuideIntakeSink {
  /**
   * Save the user's answer for a section and return the FRESH progress
   * (post-save). The tool surfaces this back to the agent so it doesn't
   * ask about the same section twice — the system-prompt progress is
   * computed at turn-start and is stale by the time this tool fires.
   */
  saveAnswer(
    userId: string,
...
export class LlmService {
    ...
    if (this.userGuideIntake) {
      tools.push({
        ...
        description:
          ...
          'The result returns the FRESH progress (answered, remaining, nextRecommendedSection) ' +
          'after this save — trust it over any earlier progress list you saw. ' +
          'Your next question to the user MUST target nextRecommendedSection, not the section you just saved.',
        input_schema: {
          type: 'object',
          properties: {
            sectionId: {
  ...
  private buildSystemPrompt(input: AnswerUserInput): string {
    ...
    const lines = [
      ...
      '',
      'Webflow-specific shortcuts you should name by default:',
      '- If the user is stuck on local dev, tool access, or environment issues, point them to the "Request Help" workflow in #triage-build-loop — that channel is faster and more reliable than I am for infra problems.',
      '- If the user asks about the Admin Panel or needs access, tell them to request VIEWER-level access in #ask-it by submitting a "Webflow Admin Tool Request". When drafting that message, use the phrase "VIEWER permission level to start" verbatim.',
      `Role track: ${profile.roleTrack}`,
  ...
  private runSaveUserGuideAnswer(
    ...
    const progress = this.userGuideIntake.saveAnswer(
      input.profile.userId,
      args.sectionId,
      args.answer
    );
    const nextRecommendedSection = progress.remaining[0] ?? null;
    return {
      saved: true,
      ...
      allSectionsAnswered: progress.remaining.length === 0,
    };
  }
...
function buildUserGuideStanza(input: AnswerUserInput): string | null {
  ...
  const isFirstTurnOfIntake = progress.answered.length === 0;

  const backgroundContext = [
    'About Webflow User Guides (for your own framing, paraphrase do not dump verbatim):',
    '- A Webflow User Guide is a personal 1-pager teammates write to tell others how they work best: schedule, communication style, what they value, what drains them, how they like feedback, how they make decisions. Teams update and refresh it about once a year.',
    '- The official template is a Google Doc at https://docs.google.com/document/d/1VUt31G4c3JrV8fzCRJEqzHkUKpwBDnpvyYNSDuOnTqI/edit — give the user this link directly on the first turn so they can open it alongside our chat.',
    '- The same template links to a Google folder with teammate examples the user can browse for inspiration.',
    '- Workflow after the intake: (1) open the template, File > Make a Copy, save it to the shared Webflow User Guide folder, (2) paste in the markdown you generate, clean up any formatting, (3) add the link to their Slack profile via "+ Add User Guide" under their profile sidebar.',
  ].join('\n');

  const firstTurnOpener = isFirstTurnOfIntake
    ? [
        'FIRST TURN OF INTAKE — open with context, then the first question:',
        '1. In 2-3 short sentences, explain what a Webflow User Guide is and why it matters (using the background above).',
        '2. Link them directly to the Google Doc template: https://docs.google.com/document/d/1VUt31G4c3JrV8fzCRJEqzHkUKpwBDnpvyYNSDuOnTqI/edit and mention they can browse teammate examples in the Google folder it links to for inspiration.',
        '3. Name that there are 8 sections and you will go one at a time.',
        '4. Ask the first question (schedule) using warm, open phrasing.',
        'Do NOT dump the full 8-section list up front. Keep it conversational.',
      ].join('\n')
    : null;
  ...
  return [
    ...
    backgroundContext,
    `Sections answered BEFORE this turn: ${describeIds(progress.answered)}.`,
    `Sections remaining BEFORE this turn: ${describeIds(progress.remaining)}.`,
    'Section catalog:',
    sectionCatalog,
    ...(firstTurnOpener ? [firstTurnOpener] : []),
    'Rules for the intake turn:',
    '- Ask about exactly ONE remaining section per turn, using warm, open phrasing (the catalog prompt is a good starting point).',
    '- After the user answers, call save_user_guide_answer({sectionId, answer}) with their raw words. Do not paraphrase.',
    '- CRITICAL: the tool result returns fresh `answered`, `remaining`, and `nextRecommendedSection`. After the save, your follow-up question MUST target `nextRecommendedSection`. Never re-ask the section you just saved.',
    '- If `allSectionsAnswered` comes back true, skip asking more and go straight to finalize_user_guide.',
    '- If the user wants to skip, re-do a section, or pause, respect that and move on.',
